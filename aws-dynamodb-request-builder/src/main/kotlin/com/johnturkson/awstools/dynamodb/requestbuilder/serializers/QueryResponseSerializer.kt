...
import com.johnturkson.awstools.dynamodb.requestbuilder.responses.QueryResponse
...
class QueryResponseSerializer<T>(private val dataSerializer: KSerializer<T>) : KSerializer<QueryResponse<T>> {
    override val descriptor: SerialDescriptor = dataSerializer.descriptor
    
    override fun serialize(encoder: Encoder, value: QueryResponse<T>) {
    ...
    override fun deserialize(decoder: Decoder): QueryResponse<T> {
        val transformer = DynamoDBTransformingSerializer(dataSerializer)
        ...
        return QueryResponse(count, items, scannedCount, lastEvaluatedKey)
    }
}
