...
import com.johnturkson.awstools.ses.requestbuilder.components.Body
import com.johnturkson.awstools.ses.requestbuilder.components.Body.HtmlBody
import com.johnturkson.awstools.ses.requestbuilder.components.Body.TextBody
    ...
    override fun deserialize(decoder: Decoder): Body {
        return decoder.decodeStructure(descriptor) {
            when (val index = decodeElementIndex(descriptor)) {
                0 -> HtmlBody(decodeSerializableElement(Content.serializer().descriptor, index, Content.serializer()))
                1 -> TextBody(decodeSerializableElement(Content.serializer().descriptor, index, Content.serializer()))
